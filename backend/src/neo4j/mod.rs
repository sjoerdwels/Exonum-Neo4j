...
impl Neo4jRpc {
    ...
    pub fn new(config: Neo4jConfig) -> Self {
        ...
        let grpc_client =
            Arc::new(Client::new_plain(&config.address, config.port, client_conf).unwrap());
    ...
    pub fn execute_block(
        ...
        block: &Block,
        block_hash: &str,
        core_schema: &CoreSchema<&dyn Snapshot>,
        ...
        for trans_hash in transactions.iter() {
            ...
            if let Some(neo4j_transaction) = potential_trans {
                ...
                let query_vector: Vec<::std::string::String> =
                    queries.map(|s| s.to_string()).collect();
        ...
        if !trans_vector.is_empty() {
            ...
            let resp = self
                .transaction_manager
                .execute_block(RequestOptions::new(), request);

            let result = resp.wait();
            match result {
                Ok(_) => OkExe(()),
                Err(e) => ExecuteResponse::Error(ErrorMsg::new(format!("{:?}", e).as_str())),
            }
        } else {
            NoCommits(())
        }
    ...
    pub fn remove_audited_changes(
        ...
        for trans_hash in transactions.iter() {
            ...
            for block in audited_blocks.iter() {
                ...
                let resp = self
                    .transaction_manager
                    .delete_block_changes(RequestOptions::new(), request);
                match resp.wait() {
                    Ok(x) => {
                        if x.1.get_success() {
                            println!(
                                "Succesfully deleted changes in neo4j for {}",
                                block.to_hex().as_str()
                            );
                        } else {
                            println!(
                                "Failed to deleted changes in neo4j for {}",
                                block.to_hex().as_str()
                            );
                        }
                    }
    ...
    pub fn retrieve_block_changes(&self, block_hash: Hash) -> ExecuteResponse {
        ...
        let resp = self
            .transaction_manager
            .retrieve_block_changes(RequestOptions::new(), request);

        let result = resp.wait();
        match result {
            Ok(x) => ChangeResponse(x.1),
            Err(e) => ExecuteResponse::Error(ErrorMsg::new(format!("{:?}", e).as_str())),
        }
    }
}
...
pub fn generate_database_changes_from_proto(
    modifs: &DatabaseModifications,
    ...
    for new_label in modifs.get_assigned_labels() {
        let new_change = AddLabel::new(
            new_label.get_node_UUID(),
            new_label.get_name(),
            transaction_id,
        );
    ...
    for remove_label in modifs.get_removed_labels() {
        let new_change = RemoveLabel::new(
            remove_label.get_node_UUID(),
            remove_label.get_name(),
            transaction_id,
        );
        changes.push(RL(new_change));
    }
    for new_node_property in modifs.get_assigned_node_properties() {
        let new_change = AddNodeProperty::new(
            new_node_property.get_node_UUID(),
            new_node_property.get_key(),
            new_node_property.get_value(),
            transaction_id,
        );
        changes.push(ANP(new_change));
    }
    for remove_node_property in modifs.get_removed_node_properties() {
        let new_change = RemoveNodeProperty::new(
            remove_node_property.get_node_UUID(),
            remove_node_property.get_key(),
            transaction_id,
        );
        changes.push(RNP(new_change));
    }
    for new_relation in modifs.get_created_relationships() {
        let new_change = AddRelation::new(
            new_relation.get_relationship_UUID(),
            new_relation.get_field_type(),
            new_relation.get_start_node_UUID(),
            new_relation.get_end_node_UUID(),
            transaction_id,
        );
        changes.push(AR(new_change));
        let r: Relation = Relation::new(
            new_relation.get_start_node_UUID(),
            new_relation.get_end_node_UUID(),
        );
    ...
    for new_relation_property in modifs.get_assigned_relationship_properties() {
        match schema.relation(new_relation_property.get_relationship_UUID()) {
            Some(relation) => {
                let new_change = AddRelationProperty::new(
                    new_relation_property.get_relationship_UUID(),
                    new_relation_property.get_key(),
                    new_relation_property.get_value(),
    ...
    for remove_relation_property in modifs.get_removed_relation_properties() {
        match schema.relation(remove_relation_property.get_relationship_UUID()) {
            Some(relation) => {
                let new_change = RemoveRelationProperty::new(
                    remove_relation_property.get_relationship_UUID(),
                    remove_relation_property.get_key(),
                    relation.start_node_uuid(),
                    relation.end_node_uuid(),
                    transaction_id,
                );
            ...
            _ => {
                println!("ERROR: Should not be here 005");
            }
        }
    }
    changes
